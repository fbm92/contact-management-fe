...
import { deleteAddress, getAddress } from "../../lib/api/AddressApi";
...
export const ContactDetail = () => {
  ...
  async function handleDeleteAddress(addressId) {
    if (!(await alertConfirm("apakah yakin akan didelete ?"))) {
      return;
    }
    ...
    if (response.status === 200) {
      await alertSuccess("Alamat Berhasil di Hapus");
  ...
  async function getListAddress() {
    const response = await getAddress(token, id);
    const responseBody = await response.json();
    console.log(responseBody);
  ...
  useEffectOnce(() => {
    getContactById().then(() => console.log("detail contact fetched"));

    getListAddress().then(() => console.log("list address fethced"));
  });
          ...
          <div className="p-8">
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {address.map((address) => (
                  <AddressCard key={address} address={address}  handleDelete={handleDeleteAddress} id={id}/>
