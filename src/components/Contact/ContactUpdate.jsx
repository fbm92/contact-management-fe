...
import { detailContactId, updateContact } from "../../lib/api/ContactApi";
...
export const ContactUpdate = () => {
  ...
  async function handleSubmit(e) {
    ...
    if (response.status === 200) {
      const response = await updateContact(token, { id, first_name, last_name, email, phone });
      const responseBody = await response.json();
      console.log(responseBody);

      if (response.status === 200) {
        await alertSuccess("Data Berhasil di Ubah");
        await navigate({
  ...
  useEffectOnce(() => {
    getDetail().then(console.log("detail fetched"));
  });
  ...
  return (
    <>
      ...
      <ContactForm paramsId={id} first_name={first_name} last_name={last_name} email={email} phone={phone} setFirstname={setFirstName} setLastname={setLastName} setEmail={setEmail} setPhone={setPhone} handleSubmit={handleSubmit} />
    </>
  );
};
